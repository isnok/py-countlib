...
class Counter(dict):
    ...
    def most_common(self, n=None, count_func=None, inverse=False):
        ...
        if count_func is None:
            count_func = _itemgetter(1)
        if n is None:
            return sorted(self.iteritems(), key=count_func, reverse=not inverse)
        if inverse:
            return _heapq.nsmallest(n, self.iteritems(), key=count_func)
        else:
            return _heapq.nlargest(n, self.iteritems(), key=count_func)

    def most_common_counts(self, n, count_func=None, inverse=False):
