...
class Counter(dict):
    ...
    def __getitem__(self, key):
        """ Like dict.__getitem__() but does not raise KeyError for missing values.
            Also slicing is supported and applied to count ranges.
            The range can also be inverted by using -1 as the step value.
        """
        try:
            return super(Counter, self).__getitem__(self, key)
        except TypeError, ex:
    ...
    def __delitem__(self, key):
        ...
        try:
            if key in self:
                return super(Counter, self).__delitem__(self, key)
        except TypeError, ex:
            if ex.message == 'unhashable type' and isinstance(key, slice):
                start, stop, step = key.start, key.stop, key.step
                if step is None:
                    if start is not None and stop is not None:
                        for k, v in [ i for i in self.iteritems() if start <= i[1] < stop ]:
                            super(Counter, self).__delitem__(self, k)
                    if start is not None and stop is None:
